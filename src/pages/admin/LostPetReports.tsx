...
export default function LostPetReports() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      setError(null);
      const res = await fetchAdminLostReports(status);
      if (!mounted) return;
  ...
  async function changeStatus(id: number, next: string) {
    ...
    setError(null);
    const res = await updateAdminLostReport(id, { status: next });
    if (!res.ok) {
            ...
            <Paper key={r.id} sx={{ p: 2, display: "grid", gridTemplateColumns: "80px 1fr auto", gap: 2 }}>
              ...
              <Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.5 }}>
                  <Chip label="LOST" color="warning" size="small" />
                  <Chip label={r.status} size="small" />
                </Box>
                <Typography variant="h6" sx={{ mb: 0.5 }}>
                  {r.pet_name || r.pet_type}
                </Typography>
                <Typography variant="body2" sx={{ color: "text.secondary" }}>
                  Breed: {r.breed || "-"}
                </Typography>
                <Typography variant="body2" sx={{ color: "text.secondary" }}>
                  Last Seen Location: {r.city}, {r.state}
                </Typography>
                <Typography variant="body2" sx={{ color: "text.secondary" }}>
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                <Button
                  variant="outlined"
                  size="small"
                  onClick={() => navigate(`/admin/lost/${r.id}`)}
                >
                  View
                </Button>
                <Button
