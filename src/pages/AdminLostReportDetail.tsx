      ...
      {hasUpdate ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "280px 1fr 1fr", gap: 20, alignItems: "start" }}>
          ...
          <div style={{ background: "white", borderRadius: 20, padding: 12, boxShadow: "0 10px 30px rgba(15,23,42,0.1)" }}>
            {photoSrc ? (
              <img src={photoSrc} alt={report.pet_name || "Pet"} style={{ width: "100%", height: 300, borderRadius: 14, objectFit: "cover" }} />
            ) : (
            ...
            <div style={{ marginTop: 12 }}>
              <div style={{ display: "flex", gap: 6, marginBottom: 6, alignItems: "center", flexWrap: "wrap" }}>
                <span style={{ padding: "3px 8px", borderRadius: 999, background: "#fee2e2", color: "#b91c1c", fontSize: 11, fontWeight: 800 }}>Lost Pet</span>
              ...
              <div style={{ fontSize: 11, color: "#6b7280" }}>Pet ID: <span style={{ fontWeight: 700, color: "#111827" }}>#{report.pet_unique_id || `LP${report.id?.toString().padStart(6, "0")}`}</span></div>
              <div style={{ fontSize: 20, fontWeight: 900, color: "#0f172a" }}>{report.pet_name || report.pet_type || "Lost Pet"}</div>
              {report.city && <div style={{ color: "#6b7280", fontSize: 12 }}>{report.city}{report.state ? `, ${report.state}` : ""}</div>}
            </div>
          </div>
            ...
            <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
              <div style={{ background: "#f9fafb", borderRadius: 16, padding: 14, boxShadow: "0 6px 20px rgba(15,23,42,0.08)", border: "1px solid rgba(226,232,240,0.9)", flex: 1 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  ...
                  {report.lost_time && (
                    <div style={{ background: "#fff", borderRadius: 10, padding: 8, boxShadow: "0 2px 6px rgba(148,163,184,0.2)" }}>
                      <div style={{ fontSize: 9, color: "#6b7280", textTransform: "uppercase", fontWeight: 700, marginBottom: 2 }}>Lost time</div>
                      <div style={{ fontSize: 11, fontWeight: 500, color: "#111827" }}>{new Date(report.lost_time).toLocaleString()}</div>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 2fr) minmax(0, 3fr)", gap: 24, alignItems: "start" }}>
          ...
          <div style={{ background: "white", borderRadius: 24, padding: 16, boxShadow: "0 20px 50px rgba(15,23,42,0.12)" }}>
            {photoSrc ? (
              <img src={photoSrc} alt={report.pet_name || "Pet"} style={{ width: "100%", borderRadius: 18, objectFit: "cover" }} />
            ) : (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
                <span style={{ padding: "4px 10px", borderRadius: 999, background: "#fee2e2", color: "#b91c1c", fontSize: 12, fontWeight: 800 }}>Lost Pet</span>
              ...
              <div style={{ fontSize: 12, color: "#6b7280" }}>Pet ID: <span style={{ fontWeight: 700, color: "#111827" }}>#{report.pet_unique_id || `LP${report.id?.toString().padStart(6, "0")}`}</span></div>
              <div style={{ fontSize: 28, fontWeight: 900, color: "#0f172a" }}>{report.pet_name || report.pet_type || "Lost Pet"}</div>
              {report.city && <div style={{ color: "#6b7280", fontSize: 14 }}>{report.city}{report.state ? `, ${report.state}` : ""}</div>}
            </div>
            ...
            <div style={{ background: "linear-gradient(135deg, rgba(219,234,254,0.85), rgba(239,246,255,0.9))", borderRadius: 24, padding: 18, boxShadow: "0 14px 40px rgba(15,23,42,0.16)", border: "1px solid rgba(148,163,184,0.35)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, background: "rgba(255,255,255,0.8)", borderRadius: 18, padding: 14 }}>
                ...
                {renderFieldCard("Lost Time", report.lost_time ? new Date(report.lost_time).toLocaleString() : null, "⏱️")}
              </div>
            ...
            <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
              <div style={{ background: "#f9fafb", borderRadius: 18, padding: 14, boxShadow: "0 8px 24px rgba(15,23,42,0.08)", border: "1px solid rgba(226,232,240,0.9)", flex: 1 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
                  ...
                  {report.lost_time && (
                    <div style={{ background: "#fff", borderRadius: 12, padding: 10, boxShadow: "0 2px 6px rgba(148,163,184,0.2)" }}>
                      <div style={{ fontSize: 10, color: "#6b7280", textTransform: "uppercase", fontWeight: 700, marginBottom: 2 }}>Lost time</div>
                      <div style={{ fontSize: 12, fontWeight: 500, color: "#111827" }}>{new Date(report.lost_time).toLocaleString()}</div>
                    </div>
                  )}
                </div>
              </div>
