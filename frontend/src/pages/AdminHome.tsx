...
const AnyPopup = RL.Popup as any;
  ...
  function MapPins({
    ...
    useEffect(() => {
      ...
      async function load() {
        ...
        const addItem = async (entry: any, kind: "lost" | "found" | "adoption") => {
          if (kind === "lost") {
            ...
            next.push({
              id: entry.id,
              kind,
              title: entry.pet_name || entry.pet_type || "Pet",
              subtitle: entry.pet_type,
              city: entry.city,
              state: entry.state,
              photo: normalizeUrl(entry.photo_url || entry.photo),
              lat: g.lat,
              lon: g.lon,
            });
          } else if (kind === "found") {
            ...
            next.push({
              id: entry.id,
              kind,
              title: entry.pet_type || "Pet",
              subtitle: entry.color || entry.breed,
              city: entry.found_city,
              state: entry.state,
              photo: normalizeUrl(entry.photo_url || entry.photo),
              lat: g.lat,
              lon: g.lon,
            ...
            next.push({
              ...
              photo: normalizeUrl(entry.photos),
              lat: g.lat,
              lon: g.lon,
            });
          }
        ...
        if (lostRes.ok) {
          for (const r of (lostRes.data as any[]) || []) await addItem(r, "lost");
        }
        if (foundRes.ok) {
          for (const r of (foundRes.data as any[]) || []) await addItem(r, "found");
        }
        if (adoptRes.ok) {
          for (const p of (adoptRes.data as any[]) || []) await addItem(p, "adoption");
        }
    ...
    function makeIcon(url: string | null | undefined, kind: "lost" | "found" | "adoption") {
      const border = pinColor(kind);
      const safeUrl = url || "";
      const html = `
        <div style="width:40px;height:40px;border-radius:999px;border:3px solid ${border};box-shadow:0 0 0 2px rgba(255,255,255,0.9);overflow:hidden;background:#f3f4f6">
          ${safeUrl ? `<img src="${safeUrl}" style="width:100%;height:100%;object-fit:cover"/>` : `<div style=\"width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:12px;color:#111827\">${kind[0].toUpperCase()}</div>`}
        </div>`;
      ...
      <AnyMapContainer center={[20.5937, 78.9629]} zoom={5} style={{ height }}>
        ...
        {filteredItems.map((it) => (
          <AnyMarker key={`${it.kind}-${it.id}`} position={[it.lat, it.lon]} icon={makeIcon(it.photo, it.kind)}>
            <AnyPopup>
              <div style={{ width: 180 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#111827", marginBottom: 6 }}>{it.title}</div>
                <div style={{ fontSize: 12, color: "#374151", marginBottom: 6 }}>{it.subtitle || ""}</div>
                <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 10 }}>{[it.city, it.state].filter(Boolean).join(", ")}</div>
                <button
                  onClick={() => {
                    if (it.kind === "lost") navigate(`/admin/lost/${it.id}`, { state: { from: "admin-map" } });
                    else if (it.kind === "found") navigate(`/admin/found/${it.id}`, { state: { from: "admin-map" } });
                    else navigate(`/pets/${it.id}`, { state: { from: "admin-map" } });
                  }}
                  style={{ width: "100%", padding: "6px 8px", borderRadius: 8, border: "none", background: pinColor(it.kind), color: "white", fontSize: 12, fontWeight: 700, cursor: "pointer" }}
                >
                  View details
                </button>
              </div>
